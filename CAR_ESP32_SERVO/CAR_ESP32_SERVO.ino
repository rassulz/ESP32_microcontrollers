...
void handleRoot() {
  ...
  <head>
    ...
    <script>
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        // Add event listener for the servo button
      ...
      function moveKnob(event) {
        ...
        const distance = Math.min(Math.sqrt(x * x + y * y), 80);
      ...
      function resetKnob() {
          ...
          knob.style.top = "60px";
          lastDirection = "";
          stopRobot(); // Немедленная остановка робота
      }
  ...
  <body>
    ...
    <!-- Button to control the servo -->
    <button id="servoButton">Move Servo Up and Down</button>
  </body>
  </html>)rawliteral";
...
void handleLeft() {
  ...
  analogWrite(enable1Pin, 200); // Set speed for motor 1
  analogWrite(enable2Pin, 200); // Set speed for motor 2
  digitalWrite(motor1Pin1, LOW); // Motor 1 reverse
...
void handleRight() {
  ...
  digitalWrite(motor1Pin2, LOW);
  digitalWrite(motor2Pin1, LOW); // Motor 2 reverse
